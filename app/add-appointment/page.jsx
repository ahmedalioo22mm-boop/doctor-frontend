  ...
  useEffect(() => {
        const fetchDoctor = async () => {
            try {
                ...
                // The API might return an object with a 'doctors' property
                // Or it might return the array directly. We need to handle both.
                if (Array.isArray(data)) {
                    setDoctors(data);
                } else if (data && Array.isArray(data.doctors)) {
                    // If the data is { doctors: [...] }
                    setDoctors(data.doctors);
                } else {
                    console.error("API response is not an array:", data);
                    setDoctors([]); // Set to empty array if the data is not in the expected format
                }
            } catch (error) {
                console.error("Failed to fetch doctors:", error);
                setDoctors([]); // Also set to empty array on fetch error
            }
        };
  ...
  return (
    <div className='flex justify-center items-center h-screen bg-gray-100'>
        <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-8 w-full max-w-md">
            ...
            <button 
                type='submit' 
                className='w-full py-2 rounded bg-blue-800  text-white font-semibold transition duration-200'
            >
                Submit
            </button>     
        </form>
